...
public class CardsTag extends TagSupport {
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            out.print("<div class='row row-cols-1 row-cols-md-4'>");
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        for (Show show : showList) {
            ...
            try {
                ...
                for (int i = 10; i >= 1; i--) {
                    out.print(String.format("<span class='%d' onmouseout='setActive(%d, %d)' onmouseover='setActive(%d, %d)' onclick='sendRate(%d, %d)'><i class='fa fa-star'></i></span>",
                            i, show.getId(), userRate, show.getId(), i, show.getId(), i));
                }
                out.print("</div>");
                if(userRate != 0) {
